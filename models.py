...
class WFMachine(object):
    ...
    def login(self, username=None, password=None, machinename=None):
        ...
        try:
            if not self.is_test:
                ...
                kwargs = {'session': 'sessing_id_test', 'username': self.username, 'home': '/home', 'web_server': self.machinename, 'mail_server': 'test'}
                self.user = WFSession(**kwargs)
    ...
    def clear_pgpassfile(self, dbusername):
        kw = {'path': self.user.home,'username': dbusername}
        self.system("cd && cat %(path)s/.pgpass | grep -v %(username)s | grep localhost >> %(path)s/.pgpass2;mv %(path)s/.pgpass2 %(path)s/.pgpass" % kw)
...
"""
    ...
    def delete_email(self, name):
          self.check_connection()
          try:
              self.server.delete_email(self.user(), name)
              self.raise_success('Email "%s" was deleted.' % name)
          except xmlrpclib.Fault, error:
              self.raise_log('WebFaction Error >>> "%s".' % (error))


    def delete_mailbox(self, name):
          self.check_connection()
          try:
              self.server.delete_mailbox(self.user(), name)
              self.raise_success('Mailbox "%s" was deleted.' % name)
          except xmlrpclib.Fault, error:
              self.raise_log('WebFaction Error >>> "%s".' % (error))
    ...
    def delete_app(self, name):
          self.check_connection()
          try:
              self.server.delete_app(self.user(), name)
              self.raise_success('Application "%s" was deleted.' % name)
          except xmlrpclib.Fault, error:
              self.raise_log('WebFaction Error >>> "%s".' % (error))
    ...
    def delete_website(self, name):
          self.check_connection()
          try:
              self.server.delete_website(self.user(), name)
              self.raise_success('WebSite "%s" was deleted.' % name)
          except xmlrpclib.Fault, error:
              self.raise_log('WebFaction Error >>> "%s".' % (error))


    def delete_db(self, name, dbtype):
          self.check_connection()
          try:
              self.server.delete_db(self.user(), name, dbtype)
              self.raise_success('Database "%s" was deleted.' % name)
          except xmlrpclib.Fault, error:
              self.raise_log('WebFaction Error >>> "%s".' % (error))
